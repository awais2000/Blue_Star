...
import fakeInvoice from "../models/fakeInvoice"
import fakeSalesDetail from "../models/fakeSalesDetails"
...
export const fcreateSaleData = async (
  ...
  try {
    ...
    let invoice = await fakeInvoice.findOne();
    ...
    if (invoice) {
      ...
      currentInvoiceNo = 1;
      invoice = await fakeInvoice.create({ invoiceNo: currentInvoiceNo });
    }
...
    const getSalesData = await (fakeSalesDetail as any).findOne({ invoiceNo: invoiceNo })
...
    const date = new Date(getSalesData.date).toLocaleString();
...
export const fgetSalesData = async (
  req: express.Request,
  res: express.Response
): Promise<void> => {
  try {
    ...
    // --- Fetch ---
    const getAllInvoices = await fakeSalesDetail.find(query)
      .populate("products.productId")
      .sort({ invoiceNo: 1 })
      .lean()
      .skip(offset)
